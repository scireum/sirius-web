...
public class LocalRenderContext {
    ...
    private Position positin = Position.UNKNOWN;
    ...
    public void output(String content) {
        globalContext.output.accept(content);
    }
    ...
    public void emitBlock(String name) {
        if (blocks == null) {
            return;
        }
    ...
    public String toString() {
        ...
        while (ctx != null) {
            sb.append(String.format("%3d:%2d: %s\n", ctx.positin.getLine(), ctx.positin.getPos(), ctx.template));
            ctx = ctx.parent;
        }
        return sb.toString();
    }
